...
class Message {
  ...
  queryList(param) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // `SELECT * FROM article order by is_top desc, create_time desc LIMIT ${pageRow * (pageNum -1)}, ${pageRow}`
      // `SELECT a.*, b.title FROM message a LEFT JOIN article b ON a.article_id = b.id order by create_time desc limit ${pageRow * (pageNum -1)}, ${pageRow}`
      pool.query(`SELECT a.*, b.title FROM message a LEFT JOIN article b ON a.article_id = b.id ${queryArticleId} order by create_time desc limit ${pageRow * (pageNum - 1)}, ${pageRow}`, (e, res, fields) => {
        if (e) {
